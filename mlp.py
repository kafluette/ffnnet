...
def build_ffnet(input, d, H, PI, rng):
    ...
    for i in xrange(d):
        ...
        g_frob = (1 / numpy.sqrt((numpy.linalg.norm(G.get_value(borrow=True),
                                                    ord='fro'))))
        area = (1.0 / numpy.sqrt(d * numpy.pi)) * \
            ((2 * numpy.pi * numpy.exp(1)) / d) ** (d / 2)
        ...
        phi_exp = phi_exp % (2*numpy.pi)  # modulo
        # M*e^(jtheta) = Mcos(theta) + jMsin(theta)
        # so don't need (1 / numpy.sqrt(m)) * T.exp(1j * phi_exp)
        phi = 1/numpy.sqrt(m)*T.sin(phi_exp)
        if print_on:
...
class MLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, d):
        ...
        # generate ffnnet parameters perm_matrix (random permutation matrix)
        # and H (hadamard matrix)
        ...
        perm_matrix_values = numpy.identity(d)
        rng.shuffle(numpy.transpose(perm_matrix_values))
        ...
        self.hiddenLayer = HiddenLayer(
            ...
            H=H,
            PI=perm_matrix,
            activation=T.tanh  # if old_method else T.nnet.sigmoid
        )
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
    ...
    print 'size of x ', numpy.size(train_set_x.eval(), axis=0),\
        numpy.size(train_set_x.eval(), axis=1)
    ...
    # so round up the input size to the next highest power of 2 and pad with
    # zeros as needed
    ...
    y = theano.printing.Print('y =')(T.ivector('y'))
